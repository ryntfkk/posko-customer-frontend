      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        {jobs.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200">
            ...
            <p className="text-sm text-gray-500 mt-1">Terima order masuk di Beranda untuk memulai.</p>
            <Link href="/" className="mt-4 inline-block text-red-600 font-bold text-sm hover:underline">Ke Beranda</Link>
          </div>
        ) : (
          jobs.map((job) => (
            <Link href={`/provider/jobs/${job._id}`} key={job._id} className="block bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-red-200 hover:shadow-md transition-all relative overflow-hidden">
