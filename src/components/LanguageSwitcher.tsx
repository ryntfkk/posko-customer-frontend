...
function LocationMarker({ 
  ...
  onLocationSelect: (lat: number, lng: number) => void,
  onEndLocate: () => void,
  // [FIX] Use L.ErrorEvent instead of any
  onError: (e: L.ErrorEvent) => void,
  initialLat?: number,
  initialLng?: number
}) {
  ...
  useEffect(() => {
    if (initialLat && initialLng) {
      // [FIX] Check if position actually needs updating to avoid set-state-in-effect warning
      if (position && position.lat === initialLat && position.lng === initialLng) {
        return;
      }
