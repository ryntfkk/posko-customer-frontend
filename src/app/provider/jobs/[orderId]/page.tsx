...
export default function ProviderJobDetail() {
  ...
  // Handler Update Status (Jalan -> Kerja -> Selesai)
  const handleUpdateStatus = async (newStatus: string) => {
    // Konfirmasi aksi agar tidak kepencet
    const actionName = newStatus === 'completed' ? 'Menyelesaikan Pekerjaan' : newStatus.replace(/_/g, ' ');
    if (!confirm(`Apakah Anda yakin ingin mengubah status menjadi "${actionName}"?`)) return;
    
    setIsUpdating(true);
    try {
      // Panggil API update status
      // Jika newStatus = 'completed', Backend akan otomatis mengubahnya jadi 'waiting_approval' dulu
  ...
  // Handler Chat dengan Customer
  const handleChat = async () => {
    if (!order) return;
    
    // Casting 'any' karena TypeScript mungkin belum mendeteksi populate userId di dalam object order
    // Pastikan backend mengirim object user lengkap di field userId
    const customerData = order.userId as any;
    const targetUserId = customerData?._id || customerData;
    ...
    try {
        // Buat room chat baru atau dapatkan yang sudah ada
        await api.post('/chat', { targetUserId });
        // Arahkan ke halaman list chat (room terbaru akan muncul paling atas)
        router.push('/chat');
    } catch(e) {
        console.error(e);
  ...
  const renderActionButton = () => {
    // 1. Status: Diterima -> OTW
    if (order.status === 'accepted') {
      return (
        <button 
          onClick={() => handleUpdateStatus('on_the_way')}
          disabled={isUpdating}
          className="w-full py-3.5 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-xl shadow-lg shadow-orange-200 transition-all active:scale-95 flex justify-center items-center gap-2"
        >
          {isUpdating ? 'Memproses...' : (
            <>🚀 Berangkat ke Lokasi</>
          )}
        </button>
      );
    ...
    // 2. Status: OTW -> Mulai Kerja
    if (order.status === 'on_the_way') {
      return (
        <button 
          onClick={() => handleUpdateStatus('working')}
          disabled={isUpdating}
          className="w-full py-3.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl shadow-lg shadow-purple-200 transition-all active:scale-95 flex justify-center items-center gap-2"
        >
          {isUpdating ? 'Memproses...' : (
            <>🛠️ Mulai Pengerjaan</>
          )}
    ...
    // 3. Status: Sedang Kerja -> Selesai (Minta Konfirmasi)
    if (order.status === 'working') {
      return (
        <button 
          onClick={() => handleUpdateStatus('completed')}
          disabled={isUpdating}
          className="w-full py-3.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg shadow-green-200 transition-all active:scale-95 flex justify-center items-center gap-2"
        >
          {isUpdating ? 'Memproses...' : (
            <>✅ Pekerjaan Selesai</>
          )}
    ...
    // 4. Status: Menunggu Konfirmasi Pelanggan (Provider harus menunggu)
  ...
  // Helper akses data user (karena populate)
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 font-sans">
      ...
      <header className="bg-white shadow-sm px-4 py-4 sticky top-0 z-20 flex items-center gap-3">
        <Link href="/provider/jobs" className="p-2 -ml-2 text-gray-600 hover:bg-gray-100 rounded-full transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"/></svg>
        </Link>
        <div>
            <h1 className="text-lg font-bold text-gray-900 leading-none">Detail Pekerjaan</h1>
            <p className="text-xs text-gray-500 mt-0.5">ID: <span className="font-mono">{order._id.substring(0, 8)}...</span></p>
        </div>
      </header>
      ...
      <main className="p-4 space-y-4 max-w-xl mx-auto">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="mt-5 bg-gray-50 p-4 rounded-xl border border-gray-200 relative overflow-hidden">
                ...
                <p className="text-sm text-gray-800 font-medium ml-1 leading-relaxed">
                    {customer?.address?.detail || 'Detail jalan tidak tersedia'}, {customer?.address?.village}, {customer?.address?.city}
                </p>
                {/* Tombol Buka Maps */}
                {customer?.location?.coordinates && (
                    <a 
                        href={`https://www.google.com/maps/dir/?api=1&destination=${customer.location.coordinates[1]},${customer.location.coordinates[0]}`}
                        target="_blank"
                ...
                {order.items.map((item: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-start border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">{item.quantity} unit x Rp {new Intl.NumberFormat('id-ID').format(item.price)}</p>
                            {item.note && <p className="text-xs text-gray-500 mt-1 italic bg-yellow-50 text-yellow-700 px-2 py-1 rounded inline-block">Catatan: {item.note}</p>}
                        </div>
        ...
        {/* Action Button (Sticky di Mobile) */}
