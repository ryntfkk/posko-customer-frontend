...
export default function ProviderHome({ user }: ProviderHomeProps) {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        if (profileRes.data) {
          const blocked = (profileRes.data.blockedDates || []).map((d: string) => d.split('T')[0]);
          const booked = (profileRes.data.bookedDates || []).map((d: string) => d.split('T')[0]);
  ...
  const handleAccept = async (orderId: string) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Gagal terima order');
    } finally {
            ...
            <div className="space-y-4">
              {activeJobs.length === 0 ?  (
                ...
                activeJobs.map((order) => (
                  // [FIX] Perbaikan link dari /provider/jobs/ ke /jobs/ sesuai struktur folder (provider)
