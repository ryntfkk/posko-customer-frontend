      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-gradient-to-r from-red-700 to-red-900 rounded-3xl p-6 text-white relative overflow-hidden shadow-xl shadow-red-100">
                <div className="relative z-10 flex flex-col h-full justify-between">
                    ...
                    <div className="mt-8 grid grid-cols-3 gap-4 divide-x divide-red-600/30">
                        <div className="pr-4"><p className="text-xs text-red-200 mb-1">Rating</p><div className="flex items-center gap-1 text-lg lg:text-2xl font-bold">{rating} <StarIcon /></div></div>
                        <div className="px-4"><p className="text-xs text-red-200 mb-1">Selesai</p><p className="text-lg lg:text-2xl font-bold">{completedCount} Job</p></div>
                        <div className="pl-4"><p className="text-xs text-red-200 mb-1">Saldo</p><p className="text-lg lg:text-xl font-bold">Rp {new Intl.NumberFormat('id-ID').format(user.balance || 0)}</p></div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
                <div>
                    <div className="flex items-center gap-2 mb-4 text-gray-500"><div className="p-2 bg-gray-100 rounded-lg"><WalletIcon /></div><span className="text-xs font-bold uppercase tracking-wide">Total Pendapatan</span></div>
                    <p className="text-3xl font-black text-gray-900 mb-1">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(totalEarnings)}</p>
                    <p className="text-xs text-gray-400">Akumulasi dari pesanan selesai</p>
                </div>
                <button className="w-full mt-6 py-3 rounded-xl bg-gray-900 text-white text-sm font-bold hover:bg-gray-800 transition-colors">Riwayat Transaksi</button>
            </div>
        </section>
        ...
        <section>
            <div className="flex gap-6 border-b border-gray-200 mb-6 overflow-x-auto no-scrollbar">
                ...
                <button onClick={() => setActiveTab('active')} className={`pb-3 text-sm font-bold whitespace-nowrap border-b-2 transition-colors ${activeTab === 'active' ? 'text-red-600 border-red-600' : 'text-gray-400 border-transparent'}`}>Berlangsung <span className="ml-1 bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full text-[10px]">{activeJobs.length}</span></button>
                <button onClick={() => setActiveTab('history')} className={`pb-3 text-sm font-bold whitespace-nowrap border-b-2 transition-colors ${activeTab === 'history' ? 'text-red-600 border-red-600' : 'text-gray-400 border-transparent'}`}>Riwayat</button>
            </div>

            {activeTab === 'incoming' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {incomingOrders.length === 0 ? (
                        <div className="col-span-full py-16 text-center bg-white rounded-2xl border border-gray-100 border-dashed"><p className="text-gray-400">Belum ada pesanan masuk.</p></div>
                    ) : (
                        incomingOrders.map((order) => (
                            <div key={order._id} className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    <div className="w-10 h-10 rounded-full bg-gray-100 overflow-hidden shrink-0"><Image src={(order.userId as any)?.profilePictureUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${(order.userId as any)?.fullName}`} width={40} height={40} alt="Cust" className="object-cover"/></div>
                                    <div><h3 className="font-bold text-gray-900 text-sm">{(order.userId as any)?.fullName}</h3><p className="text-xs text-gray-500 flex items-center gap-1"><LocationIcon /> {(order.userId as any)?.address?.city || 'Lokasi...'}</p></div>
                                </div>
                                <button onClick={() => handleAccept(order._id)} disabled={!!processingId} className="w-full py-2.5 rounded-xl text-sm font-bold text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-300 transition-colors shadow-lg shadow-red-100">{processingId === order._id ? '...' : 'Terima Order'}</button>
                            </div>
                        ))
                    )}
                </div>
                    ...
                    {historyJobs.length === 0 ? (
                        ...
                        historyJobs.map((order) => (
                            <div key={order._id} className="flex justify-between items-center p-4 bg-white rounded-xl border border-gray-100">
                                <div><h4 className="text-sm font-bold text-gray-900">{order.items[0].name}</h4><p className="text-xs text-gray-500">{new Date(order.createdAt).toLocaleDateString('id-ID')}</p></div>
                                <div className="text-right"><span className={`text-[10px] px-2 py-0.5 rounded uppercase font-bold ${order.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{order.status}</span><p className="text-xs font-bold text-gray-700 mt-1">Rp {new Intl.NumberFormat('id-ID').format(order.totalAmount)}</p></div>
                            </div>
                        ))
                    )}
                </div>
